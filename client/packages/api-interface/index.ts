...
export interface ApiCaptchaSendReq {
  /** 图形验证码的key */
  captcha_key: string;
  /** 输入的图形验证码 */
...
export interface ApiUserListReq {
  /** @min 1 */
  current?: number;
  /** 查询关键字 */
  keyword?: string;
...
export interface ApiUserListRes {
  list?: ModelUser[];
  total?: number;
}

export interface ApiUserLoginByWXMpReq {
  /** code */
  code: string;
}

export interface ApiUserLoginByWXMpRes {
  token: string;
}

export interface ApiUserLoginReq {
  /** 密码 */
  password: string;
  /** 用户名 */
  username: string;
...
export interface ApiUserLoginRes {
  token: string;
}

export interface ApiUserRegisterReq {
  /** 邮箱验证码 */
  captcha: string;
  /** 邮箱 */
  email: string;
...
export interface ApiUserRegisterRes {
  token: string;
}

export interface ApiUserUpdateStatusReq {
  /** 用户 ID */
  id: number;
  /** 状态 -1封禁 1-正常 */
  status: -1 | 1;
